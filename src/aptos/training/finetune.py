...
def format_report(report: dict) -> str:
    """Markdown for reports/finetune_messidor2.md, every number from the JSON."""
    arms = [a for a in ("none", "adjudicated", "control") if a in report]
    lines = ["# Fine-tuning on adjudicated Messidor-2 labels", "",
             "Generated by `python -m aptos.training.finetune run`. Design and "
             "predictions: `docs/finetune-prediction.md`, committed before the run.", ""]
    measures = [("referable ROC AUC", "referable_auc"), ("five-way QWK", "qwk"),
                ("accuracy", "accuracy"), ("macro F1", "macro_f1"),
                ("Moderate graded below 2", "moderate_below_2"),
                ("median score, Moderate without DME", "moderate_without_dme_median_raw"),
                ("median score, Moderate with DME", "moderate_with_dme_median_raw")]
    for name in report[arms[0]]:
        lines += [f"## {name} (n = {report[arms[0]][name]['n']})", "",
                  "| measure | " + " | ".join(arms) + " |",
                  "|---|" + "---|" * len(arms)]
        for label, key in measures:
            values = [report[a][name].get(key) for a in arms]
...
def main(argv=None) -> int:
    ...
    out.with_suffix(".md").write_text(format_report(report), encoding="utf-8")
