...
scikit-learn is imported inside the two functions that use it rather than at
module level. The inference service needs only `apply_thresholds`, which is
numpy, and on a 512 MB host there is no reason to load a metrics library to
call `np.digitize`.
"""
